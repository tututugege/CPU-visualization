...
px_bool PX_SocketConnect(PX_Socket* pSocket,const px_byte host[],px_word port)
{
	...
	if (!PX_WebsocketEmConnect((PX_WebsocketEm*)pSocket->handler,em_host))
	{
		pSocket->state = PX_SOCKET_STATE_ERROR;
		return PX_FALSE;
	}
...
px_void PX_SocketUpdate(PX_Socket* pSocket)
{
	...
	if (pSocket->state == PX_SOCKET_STATE_CONNECTING)
	{
		...
		while (pSocket->send_buffer_offset)
		{
			...
			sendsize = PX_WebsocketEmWrite((PX_WebsocketEm*)pSocket->handler, pSocket->psend_buffer, sendblocksize);
			if (sendsize > 0)
			{
		...
		{
			px_int recvsize;
			recvsize = PX_WebsocketEmRead((PX_WebsocketEm*)pSocket->handler, pSocket->precv_buffer + pSocket->recv_buffer_offset, pSocket->recv_buffer_size - pSocket->recv_buffer_offset);
			if (recvsize > 0)
			{
