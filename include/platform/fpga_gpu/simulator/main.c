...
PX_OBJECT_RENDER_FUNCTION(MyRender)
{
    ...
    PX_SurfaceCover(psurface,&surface,0,0,PX_ALIGN_LEFTTOP);
}

int main()
{
  //check GPU available
  if(!PainterEngine_Initialize(PX_GPU_HDMI_OUT_WIDTH, PX_GPU_HDMI_OUT_HEIGHT))
  {
      // initializating painterengine failed
      return 0;
  }
  ...
  PX_ObjectSetRenderFunction(root,MyRender,0);
  return 0;
}